...
- ⬜ セクション縦 padding を padY2 (64/52/44px) に

### 4-3. フッター (footer.tsx)
- ⬜ 墨帯化: `--band-bg` 背景 + `--band-border` 上罫線、padding 40px 24px
- ⬜ 常にダーク配色: ロゴ明朝 17px/700/+0.05em #eae8dc (lab=#93c7a9)、コピーライト mono 12px #6b7268
- ⬜ ソーシャルアイコン: 枠 #2c332c、色 #828773、hover #93c7a9

### 4-4. カード hover 演出
- ⬜ hover 時の緑トップバー (`before:` 3px accent-gradient) の撤去 — プロトタイプに存在しない
- ⬜ タイトルの hover 矢印 (→) とアイコンの scale/rotate の撤去
- ⬜ project/article カード hover を `translateY(-2px)` に統一 (現行 -0.5=2px ✓要確認)

### 4-5. 漢字スタンプ (オプション、デフォルト OFF)
- ⬜ SectionHeader に `kanjiStamp` prop: 30×30px 角印 (accent-gradient / #f4f1e6 / rotate(-3deg) / 明朝16px)。ページ見出しは 32×32px/17px
- ⬜ 文字: Featured=作 / Articles=記 / Projects=作 / About=信・具

### 4-6. ヒーロー
- ⬜ ターミナルを墨帯へ: `--band-bg` + `--band-texture` + `--band-border`、角丸5px、信号ボタンを彩度落とし (#c0554d/#c9a24a/#5d9b72)、本文 12.5px、配色 (prompt #93c7a9 / cmd #eae8dc / highlight #b7d9c4 / success #7fc39a / dim #6b7268)
- ⬜ メッシュブロブ (radial-gradient 2個) の撤去 — プロトタイプはドットグリッド + 墨のみ
- ⬜ ink-flow 透かし (`heroInk` prop: "main" = 墨流しメイン表示 / "background" = 透かし+ターミナル、プロトタイプ default: main)
...
- ⬜ 見出し 明朝24px/700/+0.02em (現行 SectionHeader 仕様と要調整)
- ⬜ PREVIOUS/NEXT ラベル: mono 10px/+0.18em/muted + arrow アイコン
- ⬜ リストカード内タグ: 10px/padding 1px 7px の小サイズ
- ⬜ リストサムネイル幅: 110px (デスクトップ) / 100px / 80px

### その他
- ⬜ Articles リスト表示のサムネイル幅: `minmax(140px,220px)` (現行 200px 固定)、min-height 120px
- ⬜ Toolbox チップ: bg2 背景 / mono 12.5px / 角丸2px / hover accent (現行スタイル要確認)
- ⬜ noise-overlay: プロトタイプに存在しない — 撤去を検討
- ⬜ `--thumb-gradient-*` トークンと `thumbnailVariant`/`thumbnailIcon` フィールド: 墨サムネイル移行で未使用化 — 削除を検討
- ⬜ SectionHeader の deprecated `label` prop と i18n の label キーの削除
- ⬜ Prism シンタックス色: プロトタイプは緑系トーン (#93c7a9/#7fb394 等)。現行は標準 prism 色のまま — 要判断
