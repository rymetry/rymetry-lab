...
- 📝 list サムネの `sizes` 境界を range 構文に統一 (`(width < 480px) 80px, (width < 48rem) 100px, 220px`)。Tailwind v4 の `max-*` は排他レンジなので、旧 `(max-width: 768px)` だと 768px ちょうどで実サムネ 220px に対し 100px を申告し、候補が 1 段足りなかった。単位も Tailwind 側 (`max-md` = rem / `max-[480px]` = px) に合わせる
