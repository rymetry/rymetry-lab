...
- 📝 カードサムネイルは `buildCardThumbnailUrl` で表示ボックス比の imgix `fit=crop&crop=entropy` (grid 960×400 / list 480×320、情報量の多い領域を自動で残すスマートクロップ) を要求。microCMS 画像 API は imgix Rendering API 準拠 (公式ドキュメント明記・実アセットで検証済み)。運用ルール: アイキャッチは 1200×630 (セーフエリアの扱いは下項を参照)。grid のクロップ比のズレ (要求 2.4 に対し実表示ボックスは 1.92–4.90 = viewport 320px 以上) は 2026-08-02 に実入稿画像 (1200×630 ×10 + 1024×1024 ×1) で **grid の 1〜3 カラム帯**を目視評価し、見切れ・不自然な切り出しなし — 許容とする。**list は帯ごとに別クロップを要求する Art Direction に移行済み — 下記参照**
- 📝 list サムネのモバイル帯クロップ (Issue #108 / 2026-08-02 に **Art Direction で解消**): 旧実装は単一 URL で 1.5 の横長クロップを要求し、`object-cover` が縦長ボックスへ再クロップしていた (375px でサムネ 80×137 = 比 0.59、配信画像の幅 39% しか表示されず、実効密度 1.25× で DPR2 に不足)。`getImageProps()` + `<picture><source media>` で帯ごとに別クロップを要求する形に変更。表示ボックス比は全 11 記事 × 3 surface × 10 viewport で実測し、**`<480px` = 0.43–0.59 / `480–767px` = 0.83 単一 / `>=768px` は surface 依存**であることを確認した (`/articles?view=list` と Related は 1.83 単一だが、Prev/Next は `md:grid-cols-2` で圧縮され同じ viewport でも本文量により 0.92–1.83 と揺れる)。このため広帯だけプロファイルを 2 本持つ:
  - **standard** (`/articles?view=list` / Related) — 240×400 / 300×360 / **660×360**
  - **split-nav** (Prev/Next) — 240×400 / 300×360 / **480×320 (=従来値を維持)**。実効密度は `(配信幅 / 箱幅) × min(1, 箱比 / 要求比)` なので「どの箱比でも変更前を下回らない」条件は要求比 ≤ 1.5。構図は要求比が大きいほど良いため 1.5 が最適解になる
  - `media` は `(width < 480px)` と `(width >= 480px) and (width < 48rem)` の**非重複**で書く。包含関係にすると `<picture>` の先勝ち解決により配列順の入れ替えで無言に誤クロップになる (Chromium で再現確認済み)
  - 効果 (全 380 カードの before/after 実測): サムネ枠・カード高は **380/380 で完全一致**、実効密度の低下は **0 件**・改善 190 件 (+1.13〜+1.68)、DPR 未達カードは 200 → 10 件。`>=768px` の standard は元画像の可視幅が 79% → 96% に増える
  - コスト: 記事詳細ページの `>=768px` のみ画像リクエストが増える。Related (660×360) と Prev/Next (480×320) が同じソース画像を要求すると URL が分かれ、従来の重複排除が効かなくなるため。**増加分は「両プロファイルに跨って現れるソース画像の枚数」= Prev/Next が最大 2 枚なので一般上限は +2**。`getArticleRelations` の Related は現在の記事しか除外しない (`src/lib/articles/relations.ts`) ので、Prev/Next の記事が Related にも選ばれていれば**アイキャッチが記事ごとに固有でも増加する**。2026-08-02 の実データ (11 記事中 10 記事が同一の `blog-template.png`) では **+1 / 約 +4.3KB (AVIF)**。バイト数は画像内容とクロップ比に依存するので、この 4.3KB は実測の一例であって上限ではない。一覧ページ・`<768px` は増加なし
  - **入稿ルールの変更**: 従来の「中央セーフエリア推奨」は無効。`crop=entropy` は中央を保証せず情報量の多い領域を探索するヒューリスティックなので、重要情報を画像だけに依存させない・**0.60 / 0.83 / 1.50 / 1.83** の 4 比率でクロッププレビューを確認する、という運用に切り替える (1.50 は Prev/Next の split-nav が実際に配信する比率。`crop=entropy` は要求比ごとに選択領域が変わりうるので広帯の 2 比率とも確認が要る)。厳密な構図が要る場合は将来 focal point かモバイル別画像を検討する
- 📝 list サムネの `sizes` 境界を range 構文に統一 (`(width < 480px) 80px, (width < 48rem) 100px, 220px`)。Tailwind v4 の `max-*` は排他レンジなので、旧 `(max-width: 768px)` だと 768px ちょうどで `/articles?view=list` の実サムネ 220px に対し 100px を申告し、候補が 1 段足りなかった。単位も Tailwind 側 (`max-md` = rem / `max-[480px]` = px) に合わせる。**220px は `minmax(140px,220px)` の上限であって surface ごとに実寸が違う** — `/articles?view=list` は常に 220px だが、記事詳細の Prev/Next は `max-w-[1040px]` 内の `md:grid-cols-2` で圧縮され innerWidth 768px で 145px / 1280px で 220px (2026-08-02 実測)。`sizes` は共有なので広い側 (220px) に合わせる。Prev/Next では過大申告になるが、狭い側に合わせると一覧が過小申告でぼやけるため。※ Issue #108 以降、この `sizes` 文字列は ink フォールバック専用。実画像は `<picture>` の帯ごとに `80px` / `100px` / `220px` を個別宣言する
