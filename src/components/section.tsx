...
export function SectionHeader({ label, title, description, descriptionEn, className }: SectionHeaderProps) {
  return (
    <div className={cn('mb-10 max-md:mb-7', className)}>
      ...
      {(descriptionEn ?? description) && (
        <p className="max-w-[600px] text-[15px] leading-[1.7] text-muted-foreground">
          {descriptionEn && (
            <span className="font-medium text-foreground">{descriptionEn}</span>
          )}
          {descriptionEn && description && ' — '}
          {description}
        </p>
